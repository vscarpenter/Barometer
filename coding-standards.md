...
**Portability (Claude Code and Codex).** The engineering principles in Parts 1 through 8 and Part 10 are model-agnostic and belong in both `CLAUDE.md` and a Codex `AGENTS.md`. The harness mechanics are Claude Code specific and are tagged where they appear. When generating an `AGENTS.md` for Codex, omit Part 0, Part 9, the environment-specific verification surfaces in Part 1, and the hook examples. Codex does not share those primitives or parameters, and carrying them over is dead weight at best and misleading at worst.
